...
describe('Tasks Management', () => {
  ...
  it('highlight currently applied filter', () => {
    ...
    cy.get('ul.filters')
      .within((filters) => {
        cy.get('a').contains('All').click().should('have.class', 'selected');
        cy.get('a').contains('Active').should('not.have.class', 'selected');
        cy.get('a').contains('Completed').should('not.have.class', 'selected');
